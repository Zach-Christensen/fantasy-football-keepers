...
type SleeperPlayer = { player_id: string; full_name?: string; first_name?: string; last_name?: string; team?: string | null };

function normalize(s: string) { return s.toLowerCase().replace(/[.’']/g, "").replace(/\b(sr|jr|iii|ii)\b/g, "").replace(/[^a-z0-9]/g, ""); }
function logo(team: string) { return `https://a.espncdn.com/i/teamlogos/nfl/500/${team.toLowerCase()}.png`; }
...
export default function KeeperApp() {
  ...
  function toggle(name: string) {
    ...
    if (selected.includes(name)) { setSelected(selected.filter(n => n !== name)); setMessage(""); return; }
    if (selected.length >= MAX_KEEPERS) { setMessage(`You can select up to ${MAX_KEEPERS} keepers.`); return; }
    if (points + player.points > MAX_POINTS) { setMessage(`${player.name} would put you over the ${MAX_POINTS}-point cap.`); return; }
    setSelected([...selected, name]); setMessage("");
  }
  ...
  async function submit() {
    ...
    } catch (e) { setMessage(e instanceof Error ? e.message : "Submission failed"); }
    finally { setLoading(false); }
  }

  return <main>
    <header className="topbar"><div className="brand"><span className="mark">K</span><div><b>KEEPER ROOM</b><span>2026 LEAGUE</span></div></div><div className="cap"><b>{MAX_POINTS}</b><span>POINT CAP</span></div></header>
    <section className="hero"><div className="eyebrow">2026 KEEPER SELECTION</div><h1>Lock in your squad.</h1><p>Choose your team, select up to three keepers, and stay at or under 8 points. You can edit your picks later.</p></section>
    ...
    <footer>Player imagery powered by Sleeper data. Team marks are used for roster identification.</footer>
  </main>;
}
